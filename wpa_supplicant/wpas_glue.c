				    ...
				    void *ctx)
{
	...
	if (result != EAPOL_SUPP_RESULT_SUCCESS) {
		...
		// smaller timeout for faster recovery on a failure
		wpa_supplicant_req_auth_timeout(wpa_s, 0, 50000);
#else
...
{
#if 0 // _SDC_
	struct wpa_supplicant *__wpa_s = wpa_s;
	// optimization -- use current_ssid if available
	if (__wpa_s->current_ssid) {
		return __wpa_s->current_ssid;
	}
#endif
...
#ifdef _SDC_
const u8 *wpa_supplicant_get_own_addr(void *ctx)
{
	struct wpa_supplicant *wpa_s = ctx;
