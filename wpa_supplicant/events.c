					   ...
					   bool poll_current)
{
...
#ifdef _SDC_
	// do not use SNR - too much variance
#else
	...
	if (wpa_s->signal_threshold && cur_level <= wpa_s->signal_threshold &&
		...
		min_diff -= 2;
#ifdef _SDC_
	if (summit_roam_delta > 0 &&
		min_diff > summit_roam_delta) {
		min_diff = summit_roam_delta;
	}
#endif
					      ...
					      int own_request, int update_only)
{
	...
	if (scan_res == NULL) {
#ifdef _SDC_
		// moved this check below
		// must not return without completing the radio work item
#else
		...
		if (wpa_s->scan_res_fail_handler) {
			...
			handler(wpa_s);
#ifdef _SDC_
		// moved this check here from above
		// don't rescan; but do complete the radio work item
		} else if (wpa_s->conf->ap_scan == 2 || ap ||
		    wpa_s->scan_res_handler == scan_only_handler)
		{
			; // scan failed, but don't rescan
#endif
		} else {
			wpa_dbg(wpa_s, MSG_DEBUG,
