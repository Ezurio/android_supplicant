				       ...
				       struct wpa_ssid *ssid)
{
...
#else
	/* correct above handling for roaming to/from 5GHz
	 * 1) use of abs() causes odd comparison
	 *    e.g. min_diff = 1, will roam when level is +/-1 but not equal?
	 * 2) need adjustment for both to and from 5GHz
	 *    e.g. else could ping-pong between 2.4G/-90 and 5G/-92?
	 */
	(void)to_5ghz;
#ifdef _SDC_
	if (laird_roam_delta > 0 &&
		min_diff > laird_roam_delta) {
		min_diff = laird_roam_delta;
	}
#endif
	if (selected->freq > 4000) {
		if (current_bss->freq < 4000) {
			/* to 5GHz from 2.4GHz */
			min_diff -= 2;
		}
	} else {
	...
	if (current_bss->level + min_diff > selected->level) {
					      ...
					      int new_scan, int own_request)
{
	...
	if (selected) {
		...
		if ( (ssid != wpa_s->current_ssid
#ifdef _SDC_
			  // BZ21032: also send deauthentication. if connected and using MFP
			  // otherwise new connection may be delayed by SA-query
				       ...
				       union wpa_event_data *data)
{
	...
	if (os_memcmp(bssid, wpa_s->bssid, ETH_ALEN) != 0) {
		...
		if (wpa_supplicant_select_config(wpa_s) < 0) {
			wpa_supplicant_deauthenticate(
				wpa_s, WLAN_REASON_DEAUTH_LEAVING);
#ifdef _SDC_
			// simulate an authentication timeout, else left with disconnect SSID set
			wpa_supplicant_req_auth_timeout(wpa_s, 0, 1000);
#endif
			return;
		}
	...
	// SDC: TBD: WAR for zero bssid ?
	// SDC: TBD: WAR for eapol before connect
	...
	if (wpa_s->key_mgmt == WPA_KEY_MGMT_NONE ||
		...
		// SDC: TBD -- need global auth timeout for configuration
#ifdef _SDC_
		extern unsigned int sdc_eap_timeout;
		wpa_supplicant_req_auth_timeout(wpa_s, sdc_eap_timeout, 0);
#else
	...
	if (wpa_s->pending_eapol_rx) {
		...
		if (age.sec == 0 && age.usec < 200000 &&
			...
			wpa_printf(MSG_DEBUG, "Discard pending EAPOL frame, age "
				"%d.%06d", (int)age.sec, (int)age.usec);
		}
#endif
				      ...
				      union wpa_event_data *data)
{
	...
	case EVENT_INTERFACE_REMOVED:
		...
		wpa_supplicant_mark_disassoc(wpa_s);
#ifdef _SDC_
		// stop scanning if card is removed
		wpa_supplicant_cancel_scan(wpa_s);
#endif
