...
static void wpa_supplicant_start_dms(struct wpa_supplicant *wpa_s);
#endif

const char *const wpa_supplicant_version =
"wpa_supplicant v" VERSION_STR "\n"
#ifdef _SDC_
"Copyright (c) 2023-2024 Ezurio LLC.\n"
#endif
...
#else
	// BZ21343: EAP-FAST blacklisting after fast-provisioning
	if (!wpa_s->eap_expected_failure &&
...
#ifdef CONFIG_SDC_DMS
	if (state == WPA_COMPLETED) {
		// connected, transmit DMS request if supported and enabled
		wpa_supplicant_start_dms(wpa_s);
	}
#endif
...
static void wpas_start_assoc_cb(struct wpa_radio_work *work, int deinit)
{
...
#ifdef _SDC_
	if (0) {
		// BZ13634: 50 would not roam -- ignore set bssid
#else
	if (bss) {
	...
	* CONFIG_DRIVER_NL80211_SYNA */
	...
	if (wpa_s->key_mgmt == WPA_KEY_MGMT_WPA_NONE) {
		...
		// SDC: TBD: global authentication timeout
...
#if defined(_SDC_)
		// SDC: global authentication timeout
		timeout = sdc_eap_timeout;
#endif
		wpa_supplicant_req_auth_timeout(wpa_s, timeout, 0);
	}
...
#ifdef _SDC_
		// do not increment interface count
		// repeated eject/insert poweroff/on resulting in increased timeout
#else
		interface_count++;
#endif
	} else
...
#ifdef _SDC_
	if (summit_read_config()) {
		wpa_dbg(wpa_s, MSG_ERROR, "Failed to read config");
	}
#endif
...
#ifdef _SDC_
		pmksa_cache_set_max_num_pmkids(wpa_s->wpa, capa.max_num_pmkids);
#endif
...
#ifdef _SDC_
	// allow faster reattempts
	// EAP-FAST provisioning restart
	// don't temporarily disable the SSID for initial failures
	if (ssid->auth_failures < 5)
		return;
#endif
