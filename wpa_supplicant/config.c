...
#if defined(_SDC_) && defined(IEEE8021X_EAPOL)
// set EAP-FAST defaults
static void wpa_config_sdc_eap_fast_defaults(struct wpa_ssid *ssid)
{
	if (ssid->eap.pac_file == NULL) {
    //TODO Rename-able / Multiple PAC files?
		wpa_config_set(ssid, "pac_file", "\"/data/Summit/defaultP.01.pac\"", 0);
	}
	if (ssid->eap.phase1 == NULL) {
		wpa_config_set(ssid, "phase1", "\"fast_provisioning=3\"", 0);
	}
}
#endif
...
#ifdef _SDC_
		if (strcmp(start, "FAST") == 0) {
			wpa_config_sdc_eap_fast_defaults(ssid);
		}
#endif
