...
#ifdef _SDC_
// support selection of middle/intermediate CA certificates
#define _SDC_MIDDLE_CERTS_
#endif
...
struct tls_connection {
	...
	SSL *ssl;
	BIO *ssl_in, *ssl_out;
#ifdef _SDC_MIDDLE_CERTS_
	int x509_pending_middle_err;	// see tls_verify_cb(), saved pending error in case overwritten
#endif
...
#ifdef _SDC_
// need to delete previous certificate chain
// otherwise - EAP-FAST provisioning fails (FB1049/BZ3388, BZ4506)
static void tls_connection_del_cert_chain(void *_ssl_ctx)
{
...
#else
	// free chain certs
	if (ssl_ctx->extra_certs) {
		wpa_printf(MSG_INFO, "TLS: Deleting chain extra certs");
...
#ifdef _SDC_
	{	// reduce hello response time by reducing SSL_connect() delay during real connection
		// do a fake SSL_connect to incur delay here
		SSL *s;
		s = SSL_new(ssl);
		if (s != NULL)
			SSL_connect(s);
		SSL_free(s);
		ERR_clear_error();	// clear any errors generated by fake SSL_connect
	}
#endif
...
#ifdef _SDC_
	tls_connection_del_cert_chain(ssl);
#endif
...
void tls_connection_deinit(void *ssl_ctx, struct tls_connection *conn)
{
	...
	if (conn->success_data) {
		...
		// BZ18398: SSL_SHUTDOWN_WHILE_INIT error
...
#if defined(OPENSSL_IS_BORINGSSL)
static
int __X509_STORE_CTX_has_cert(X509_STORE_CTX *ctx, X509 *x)
{
	STACK_OF(X509) *certs;
	X509 *xtmp = NULL;
	size_t i;
...
#else
// return 1 if certificate (x) is in the store (ctx)
static
int __X509_STORE_has_cert(X509_STORE *ctx, X509 *x)
{
	X509_OBJECT *obj;
	int ret=0;
...
// return 1 if the current_cert is in the certificate store (middle cert)
static int sdc_tls_check_middle(X509_STORE_CTX *x509_ctx)
{
	...
	x = X509_STORE_CTX_get_current_cert(x509_ctx);
	cert_store = SSL_CTX_get_cert_store(ssl_ctx);
#ifdef OPENSSL_IS_BORINGSSL
	if (__X509_STORE_CTX_has_cert(x509_ctx, x)) {
		wpa_printf(MSG_DEBUG, "TLS: verify: Found cert in store!!!");
		return 1;
	}
#else
	if (__X509_STORE_has_cert(cert_store, x)) {
		wpa_printf(MSG_DEBUG, "TLS: verify: Found cert in store!!!");
		return 1;
	}
#endif
...
static int tls_verify_cb(int preverify_ok, X509_STORE_CTX *x509_ctx)
{
...
#ifdef _SDC_MIDDLE_CERTS_
	// support for middle CA certificate
	// need to ignore missing root certificate, and see if can match part way through
	if (!preverify_ok) {
		switch (err) {
			// for matching on middle cert, ignore following errors and check for cert match
		case X509_V_ERR_SELF_SIGNED_CERT_IN_CHAIN:
		case X509_V_ERR_UNABLE_TO_GET_ISSUER_CERT:
		case X509_V_ERR_UNABLE_TO_GET_ISSUER_CERT_LOCALLY:
		case X509_V_ERR_CERT_UNTRUSTED:
			// check for a match to middle certificate part way through chain
			if (conn->x509_pending_middle_err == 0) {
				// save the pending error to be restored if fail to find middle certificate
				conn->x509_pending_middle_err = err;
			}
			wpa_printf(MSG_DEBUG, "TLS: %s -- middle cert check, defer error %d", __func__, err);
			preverify_ok = 1;
			break;
		default:
			break;
	...
	if (preverify_ok && conn->x509_pending_middle_err) {
		if (sdc_tls_check_middle(x509_ctx)) {
			// found a matching middle cert, clear error, and proceed
			wpa_printf(MSG_DEBUG, "TLS: %s - matched on middle cert, clear deferred errors.", __func__);
			conn->x509_pending_middle_err = 0;
		}
	}

	if (!preverify_ok) {
		// clear pending error -- not needed anymore as failing for another reason
		conn->x509_pending_middle_err = 0;
	} else if (depth == 0 && conn->x509_pending_middle_err) {
		// outstanding error wasn't cleared, no middle cert found --> fail
		wpa_printf(MSG_DEBUG, "TLS: %s - "
				   "no matching middle certificate found", __func__);
		preverify_ok = 0;
		err = conn->x509_pending_middle_err;
...
#ifdef _SDC_
		if (*suite == '-' && pos == buf) {
			// first entry disables a cipher, precede with "DEFAULT"
			def = ":DEFAULT";
		} else {
			def = "";
		}
		ret = os_snprintf(pos, end - pos, "%s:%s", def, suite);
#else
...
#if OPENSSL_VERSION_NUMBER >= 0x30000000L
		/* BZ24963: OpenSSL-3.1.1? - DO NOT force security level to 1
		 * -- breaks EAP-FAST TLS1.0/1.1 connections */
#else
...
#ifdef _SDC_
	// clear extra certs if set for previous
	tls_connection_del_cert_chain(data->ssl);
#endif
