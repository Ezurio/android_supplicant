...
// force earlier external authentication update for SAE
// based on Infineon patch, and required for the IF513 radio
// may also be necessary for other radios to avoid a race-condition between
// receiving an association-request, and setting the external auth
// compile switch to highlight the change, and allow reversion if needed
#define SDC_SAE_AUTH_EARLY_SET

#ifdef SDC_SAE_AUTH_EARLY_SET
		    ...
		    struct hostapd_frame_info *fi)
{
	...
	if (!is_broadcast_ether_addr(mgmt->bssid) &&
		...
		wpa_printf(MSG_INFO, "MGMT: BSSID=" MACSTR " not our address",
			   MAC2STR(mgmt->bssid));
#else
		// suppress the above log message
		// FILS discovery packets may trigger this every 50 msec
#endif
		return 0;
	}
