...
static void * eap_peap_init(struct eap_sm *sm)
{
	...
	data->peap_outer_success = 2;
#ifdef _SDC_
	/* FB1580: failures with MS NPS server -- default to no_binding */
	data->crypto_binding = NO_BINDING;
#else
	data->crypto_binding = OPTIONAL_BINDING;
#endif
...
#ifdef _SDC_
		// password failed?  request a new password.
		do_password = 1;
#endif
...
#ifdef _SDC_
	if (do_password) {
		// password failed, request password entry
		eap_sm_request_otp(sm, "Authentication failure", 22);
	}
#endif
