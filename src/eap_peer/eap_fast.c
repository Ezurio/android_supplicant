...
#ifdef _SDC_
// keep track of attempts without sucess and delete the auto pac
static int auto_pac_attempts = 0;
#endif
...
#ifdef _SDC_
	// TBD: if provisioning allowed, on repeated faiure, delete PACs
#endif
...
#ifdef _SDC_
	// have a new pac -- clear the count of attempts
	auto_pac_attempts = 0;
#endif
...
#ifdef _SDC_
		// need to send a disassociate packet to AP (PTX 9160 fix)
		ret->decision = DECISION_FAIL;
		ret->methodState = METHOD_DONE;
		return NULL;
#endif
...
#ifdef _SDC_
		// SDC: corrected the order of TLVs -- IRESULT before CRYPTO BINDING
		// SDC: ACS 3.3 fails if the order is incorrect; ACS 4.1 is more forgiving
...
#ifdef _SDC_
		auto_pac_attempts = 0;	// clear attempt count
#endif
	}
...
{
	data->current_pac = eap_fast_get_pac(data->pac,
#ifdef _SDC_
	...
	if (data->current_pac == NULL) {
		...
		data->current_pac = eap_fast_get_pac(
			data->pac,
#ifdef _SDC_
			// for machine pacs don't match identity ?
			NULL, 0,
#endif
			a_id, a_id_len,
...
#ifdef _SDC_
	if (data->provisioning_allowed) {
		if (++auto_pac_attempts > 3) {
			// PAC has failed several times, reprovision
			data->current_pac = NULL;
			wpa_printf(MSG_ERROR, "EAP-FAST: Multiple PAC failures. "
					   "Try to provision a new PAC-Key");
		}
	} else {
		auto_pac_attempts = 0;
	}
...
#ifdef _SDC_
		if (res < 0) {
			if (!data->provisioning && data->provisioning_allowed) {
				// failed with existing PAC, try provisioning a new PAC
				wpa_printf(MSG_ERROR, "EAP-FAST: Failed.  Try to provision a "
				   "new PAC-Key");
				ret->decision = DECISION_FAIL;
				ret->methodState = METHOD_DONE;
				auto_pac_attempts = 1000;
			}
		}
#endif
