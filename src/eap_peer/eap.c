...
#ifdef _SDC_
static const u8 * sdc_fast_identity(struct eap_sm *sm,
							  const u8 *identity, int plen,
							  u8 *buf);

#define EAP_FAST_IDENTITY_LENGTH 18
#endif
...
#ifdef _SDC_
	sm->onSuccessCopyNewPassword = false;
#endif
...
#ifdef _SDC_DISABLED_DUE_TO_BZ6240
	// BZ6240 - DO NOT ENABLE THE FOLLOWING
	// following causes SupplicantStateTracker to disable the wifi
	// keep this note so not reintroduced from  linux supplicant code
...
#ifdef _SDC_
	wpa_printf(MSG_DEBUG, "EAP: Success, copy new password %s",
		sm->onSuccessCopyNewPassword ? "true" : "false");
	if (sm->onSuccessCopyNewPassword) {
		struct eap_peer_config *config = eap_get_config(sm);
		sm->onSuccessCopyNewPassword = false;
		if (config && config->new_password) {
			wpa_msg(sm->msg_ctx, MSG_INFO,
				WPA_EVENT_PASSWORD_CHANGED
				"EAP: Password changed successfully");
...
#ifdef _SDC_
		// EAP_METHOD is missing an RFC4137 transition to FAILURE
		// perhaps to allow a response to be sent before failing
		// add failure transition here instead
		// needed for EAP-FAST restart after provisioning
...
struct wpabuf * eap_sm_buildIdentity(struct eap_sm *sm, int id, int encrypted)
{
	...
	size_t identity_len;
#ifdef _SDC_
	u8 identity_fast[EAP_FAST_IDENTITY_LENGTH]; // to receive EAP-FAST identity
#endif
...
#ifdef _SDC_
/**
 * eap_get_own_addr - Get mac address
 */
const u8 * eap_get_own_addr(struct eap_sm *sm)
{
	return sm->eapol_cb->get_own_addr(sm->eapol_ctx);
}

/**
 * sdc_fast_identity
 * replaces identity "FAST-000000000000" with mac address
 */
static const u8 * sdc_fast_identity(struct eap_sm *sm,
							  const u8 *identity, int len,
							  u8 *buf)
{
	const u8 *addr;
	char temp[16];
	do {
		if (len != 17) break;
		if (memcmp(identity, "FAST-000000000000", 17) != 0) break;
		addr = eap_get_own_addr(sm);
		if (!addr) break;
		sprintf(temp, "%02x%02x%02x%02x%02x%02x", 
				addr[0], addr[1], addr[2], addr[3], addr[4], addr[5]);
		memcpy(&buf[0], "FAST-", 5);
		memcpy(&buf[5], temp, 12);
		return buf;
	} while (0);
	return identity;
}
