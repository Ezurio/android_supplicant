	...
	size_t req_len, u8 id)
{
	...
	if (data->passwd_change_challenge_valid) {
		...
		challenge = pos;
#else
	// always use the challenge from the current packet
	// otherwise after a password change, reconnect ends up using the challenge from the password change!!!
	challenge = pos;
#endif
					    ...
					    size_t req_len, u8 id)
{
	...
	if (!data->auth_response_valid ||
		...
		ret->decision = DECISION_FAIL;
#ifdef _SDC_
		// TBD:SDC -- request new password here?
#endif
		return NULL;
	}
...
#ifdef _SDC_
	} else if (data->prev_error == ERROR_CHANGING_PASSWORD) {
		// new password was not acceptable -- try again
		wpa_msg(sm->msg_ctx, MSG_INFO,
				"EAP-MSCHAPV2: New password unacceptable - password "
				"change required");
		eap_sm_request_new_password(sm);

	} else if (data->prev_error == ERROR_AUTHENTICATION_FAILURE) {
		wpa_printf(MSG_ERROR, "EAP-MSCHAPV2: authentication failure");
