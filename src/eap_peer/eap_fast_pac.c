...
#ifdef _SDC_
			// return pac if no identity check, or identity matches
			if (!identity || !identity_len ||
				!pac->i_id || !pac->i_id_len ||
				(identity_len == pac->i_id_len &&
				 os_memcmp(identity, pac->i_id, identity_len) == 0))
#endif
			return pac;
		}
		pac = pac->next;
...
#ifdef _SDC_
		// BZ23652: need to mask off the upper bit of type (mandatory bit)
		type &= 0x7fff;
#endif
