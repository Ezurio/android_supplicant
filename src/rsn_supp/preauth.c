			 ...
			 int prio, int preauth)
{
	...
	if (sm->network_ctx) {
		if (sm->proactive_key_caching) {
			// never do preauthentication in OPMK
			return;
		} else {
			// SPMK -- send previous PMKID to driver
			struct rsn_pmksa_cache_entry *pmksa;
			pmksa = pmksa_cache_get(sm->pmksa, bssid, NULL, NULL, sm->network_ctx, 0);
			if (pmksa) {
				wpa_printf(MSG_DEBUG, "Previous PMKID found, send to driver");
				wpa_sm_add_pmkid(sm, NULL, bssid, pmksa->pmkid,
								 NULL, pmksa->pmk, pmksa->pmk_len,
								 0, 0, pmksa->akmp);
			}
			// if a PMKID is found, should still proceed to preauth ?
		}
	}
#endif
