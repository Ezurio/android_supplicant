...
#ifdef _SDC_
static void summit_auto_pmk(struct wpa_sm *sm,
...
#ifdef _SDC_
	// automatic PMK proactive_key_caching selection (OPMK/SPMK)
	summit_auto_pmk(sm, src_addr, pmkid);
#endif
...
#ifdef _SDC_
		// BZ21401: remove stale PMKID, else may repeat
		wpa_printf(MSG_DEBUG, "RSN: remove stale pmkid");
		wpa_sm_remove_pmkid(sm, NULL, sm->bssid, pmkid, NULL);
#endif
...
{
#ifdef _SDC_
	// multiple SSID support can result in WPAIE mismatch
	// due to WPAIE returned from beacon rather than probe response
	// therefore, do not bother checking for now
	return 0;
#else
...
void wpa_sm_notify_assoc(struct wpa_sm *sm, const u8 *bssid)
{
	...
	if (sm->fils_completed) {
		 ...
		 * AUTHENTICATED state to get the EAPOL port Authorized.
		 */
		// TBD:SUMMIT8 - did we accidentally delete a line here ??
		wpa_supplicant_key_neg_complete(sm, sm->bssid, 1);
		clear_keys = 0;
	}
#endif /* CONFIG_FILS */
...
void wpa_sm_set_config(struct wpa_sm *sm, struct rsn_supp_config *config)
{
	...
	if (config) {
#ifdef _SDC_
		int network_change = (sm->network_ctx != config->network_ctx);
#endif
		...
		// automatic PMK proactive_key_caching selection (OPMK/SPMK)
		if (network_change ||
			sm->auto_pmk.previous_proactive_key_caching !=
			config->proactive_key_caching)
		{
			// network, or configuration changed -- adopt config setting
			sm->proactive_key_caching = config->proactive_key_caching;
			// reset the auto_pmk state
			os_memset(&sm->auto_pmk, 0, sizeof(sm->auto_pmk));
		}
		// else, maintain the current setting and auto_pmk state
		// save the config value to compare next time
		sm->auto_pmk.previous_proactive_key_caching =
			config->proactive_key_caching;
		// output a message if not using configured setting
		if (!!sm->proactive_key_caching != !!config->proactive_key_caching)
			wpa_printf(MSG_DEBUG, "auto-PMK: currently using %sPMK",
					   sm->proactive_key_caching ? "O" : "S");
#endif
...
static void summit_auto_pmk(struct wpa_sm *sm,
						   ...
						   const u8 *pmkid)
{
	struct rsn_pmksa_cache_entry *entry = sm->cur_pmksa;

	if (!pmkid)
		return;
	if (!entry) {
		// find a match using just the mac address
		entry = pmksa_cache_get(sm->pmksa, src_addr, NULL, NULL, 0);
	}
	if (!entry)
		return;

	if (os_memcmp_const(pmkid, entry->pmkid, PMKID_LEN) != 0) {
		// if the pmkid doesn't match
		wpa_printf(MSG_DEBUG, "auto-PMK: %sPMK usage failed",
				   entry->orig_opportunistic ? "O" : "S");
		if (entry->orig_opportunistic) {
			if (sm->proactive_key_caching &&
				++sm->auto_pmk.opmk_fail_count >= 2)
			{
				// OMPK failures, try using SPMK
				sm->proactive_key_caching = 0;
				sm->auto_pmk.opmk_fail_count = 0;
				wpa_printf(MSG_INFO, "auto-PMK: switching to SPMK"
						   ", after OPMK failures.");
			}
		} else {
			if (!sm->proactive_key_caching &&
				++sm->auto_pmk.spmk_fail_count >= 2)
			{
				// SMPK failures, try using OPMK
				sm->proactive_key_caching = 1;
				sm->auto_pmk.spmk_fail_count = 0;
				wpa_printf(MSG_INFO, "auto-PMK: switching to OPMK"
						   ", after SPMK failures");
			}
		}
	} else {
		// if the pmkid matches
		if (entry->orig_opportunistic) {
			wpa_printf(MSG_DEBUG, "auto-PMK: OPMK usage succeeded");
			sm->auto_pmk.opmk_fail_count = 0;
			if (!sm->proactive_key_caching) {
				// OPMK succeeded, try using OPMK
				sm->auto_pmk.spmk_fail_count = 0;
				sm->proactive_key_caching = 1;
				wpa_printf(MSG_INFO, "auto-PMK: switching to OPMK"
						   ", after OPMK success");
			} else if (sm->auto_pmk.opmk_fail_count) {
				// clear OPMK failure count
				sm->auto_pmk.opmk_fail_count = 0;
				wpa_printf(MSG_DEBUG, "auto-PMK: clear OPMK fail count");
			}
		} else if (sm->auto_pmk.last_spmk.network_ctx ==
				   entry->network_ctx &&
				   os_memcmp(sm->auto_pmk.last_spmk.addr,
							 entry->aa, ETH_ALEN) != 0)
		{
			// success with older SPMK, clear fail count
			wpa_printf(MSG_DEBUG, "auto-PMK: SPMK(older) usage succeeded");
			if (sm->proactive_key_caching) {
				// SPMK(older) succeeded, try using SPMK
				sm->auto_pmk.opmk_fail_count = 0;
				sm->proactive_key_caching = 0;
				wpa_printf(MSG_INFO, "auto-PMK: switching to SPMK"
						   ", after SPMK(older) success");
			} else if (sm->auto_pmk.spmk_fail_count) {
				sm->auto_pmk.spmk_fail_count = 0;
				wpa_printf(MSG_DEBUG, "auto-PMK: clear SPMK fail count");
			}
		} else {
			// current SPMK success, or network changed
			wpa_printf(MSG_DEBUG, "auto-PMK: SPMK usage succeeded");
		}
	}
	if (!entry->orig_opportunistic) {
		// save the last SPMK address/network used
		wpa_printf(MSG_DEBUG, "auto-PMK: save last SPMK " MACSTR,
				   MAC2STR(entry->aa));
		os_memcpy(sm->auto_pmk.last_spmk.addr, entry->aa, ETH_ALEN);
		sm->auto_pmk.last_spmk.network_ctx = entry->network_ctx;
	}
}
#endif
