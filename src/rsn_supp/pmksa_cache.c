...
#ifdef _SDC_OPMK_UPDATE_
// returns true if an opmk update is required
static int
pmksa_need_opmk_update(struct rsn_pmksa_cache *pmksa,
					   const u8 *pmk, size_t pmk_len,
					   void *network_ctx);
// updates existing pmkid for opmk
static void
pmksa_opmk_update(struct rsn_pmksa_cache *pmksa,
				  struct rsn_pmksa_cache_entry *entry);
#endif
		      ...
		      struct rsn_pmksa_cache_entry *entry)
{
	...
	prev = NULL;
#ifdef _SDC_OPMK_UPDATE_
	need_opmk_update =  pmksa_need_opmk_update(pmksa, entry->pmk, entry->pmk_len,
											   entry->network_ctx);
#endif
	while (pos) {
...
#ifdef _SDC_OPMK_UPDATE_
			// don't flush the PMK cache
			// for OPMK, existing entries will be updated
			// for SPMK, entries should not be deleted
#else
	...
	if (pmksa->pmksa_count >=
#ifndef _SDC_
		pmksa_cache_max_entries
#else
		pmksa->max_num_pmkids
#endif
		&& pmksa->pmksa) {
...
#ifdef _SDC_OPMK_UPDATE_
	if (need_opmk_update) {
		// OPMK: PMK changed, update PMKIDs
		pmksa_opmk_update(pmksa, entry);
	}
#endif
...
#ifdef _SDC_
	new_entry->orig_opportunistic = 1;
#endif
...
#ifdef _SDC_
void pmksa_cache_set_max_num_pmkids(struct wpa_sm *sm,
									int max_num_pmkids)
{
	struct rsn_pmksa_cache *pmksa = sm->pmksa;
	if ((0 < max_num_pmkids) && (max_num_pmkids < pmksa_cache_max_entries))
		pmksa->max_num_pmkids = max_num_pmkids;
}
#endif
		 ...
		 void *ctx, struct wpa_sm *sm)
{
	...
	if (pmksa) {
		...
		pmksa->sm = sm;
#ifdef _SDC_
		pmksa->max_num_pmkids = pmksa_cache_max_entries;
#endif
	}
...
#ifdef _SDC_OPMK_UPDATE_
// check if need opmk update
// return true if using opmk and entry matching network_ctx with different pmk
static int
pmksa_need_opmk_update(struct rsn_pmksa_cache *pmksa,
					 const u8 *pmk, size_t pmk_len,
					 void *network_ctx)
{
	...
	if (!pmksa->sm->proactive_key_caching) return 0;
	pos = pmksa->pmksa;
	while (pos) {
		if (pos->network_ctx == network_ctx) {
			if (os_memcmp(pos->pmk, pmk, pmk_len) != 0) {
				// pmk changed -- need to update pmkids
				return 1;
			}
			return 0;
		}
...
// for opmk, update most recent four entries with same network_ctx as entry
static void
pmksa_opmk_update(struct rsn_pmksa_cache *pmksa,
				  struct rsn_pmksa_cache_entry *entry)
{
	...
	if (os_memcmp(pmksa->sm->bssid, entry->aa, ETH_ALEN) == 0) {
		// save the new SPMK mac address and network
		wpa_printf(MSG_DEBUG, "auto-PMK: new SPMK for " MACSTR, MAC2STR(entry->aa));
		os_memcpy(pmksa->sm->auto_pmk.last_spmk.addr, entry->aa, ETH_ALEN);
		pmksa->sm->auto_pmk.last_spmk.network_ctx = entry->network_ctx;
	}

	pos = pmksa->pmksa;
	while (pos) {
		if (pos->network_ctx == entry->network_ctx) {
			if (pos != entry && !pos->fils_cache_id_set) {
				// clone (opportunistic) the entry just added
				wpa_printf(MSG_ERROR, "PMK: Remove entry for "
						   MACSTR, MAC2STR(pos->aa));
				wpa_sm_remove_pmkid(pmksa->sm, pos->network_ctx, pos->aa, pos->pmkid, NULL);
				os_memcpy(pos->pmk, entry->pmk, entry->pmk_len);
				pos->pmk_len = entry->pmk_len;
				pos->akmp = entry->akmp;
				pos->opportunistic = 1;
				pos->orig_opportunistic = 1;
				rsn_pmkid(pos->pmk, pos->pmk_len, pos->aa,
						  pmksa->sm->own_addr, pos->pmkid,
						  wpa_key_mgmt_sha256(pos->akmp));
				wpa_printf(MSG_ERROR, "PMK: Add entry for "
						   MACSTR, MAC2STR(pos->aa));
				wpa_sm_add_pmkid(pmksa->sm, pos->network_ctx, pos->aa, pos->pmkid, NULL, pos->pmk, pos->pmk_len,
								 pmksa->sm->dot11RSNAConfigPMKLifetime,
								 pmksa->sm->dot11RSNAConfigPMKReauthThreshold,
								 entry->akmp);
			}
