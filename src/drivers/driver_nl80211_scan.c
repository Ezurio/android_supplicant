		    ...
		    struct wpa_driver_scan_params *params)
{
	...
	do {
		...
		if (params->freqs) {
			...
			freq_check = 1;
		} else if (laird_dfs_disable && drv->dfs_freq) {
	...
	if (laird_scan_dwell
		|| laird_scan_passive_dwell
		|| laird_scan_delay
		|| laird_scan_suspend_time)
	{
		if (0 == (drv->capa.flags2 & WPA_DRIVER_FLAGS2_SET_SCAN_DWELL)) {
			...
			os_memset(&laird_scan_data, 0, sizeof(laird_scan_data));
			laird_scan_data.duration = laird_scan_dwell;
			...
			if (nla_put(msg, NL80211_ATTR_MEASUREMENT_DURATION,
						sizeof(laird_scan_data), &laird_scan_data))
				goto fail;
		}
	}
#endif
