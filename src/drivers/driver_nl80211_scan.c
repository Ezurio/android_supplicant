...
#ifdef _SDC_
// restrict scan channels to allowed channels
static int
_sdc_allowed_freq(struct wpa_driver_nl80211_data *drv,
				  int freq)
{
	if (drv->all_freq && !int_array_has(drv->all_freq, freq))
		return 0;
	if (summit_dfs_disable) {
		if (drv->dfs_freq && int_array_has(drv->dfs_freq, freq))
			return 0;
	}
	return 1;
}
		    ...
		    struct wpa_driver_scan_params *params)
{
	...
	do {
		...
		if (params->freqs) {
			// requested frequency list
			pfreq = params->freqs;
			// check frequencies - exclude dfs/unsupported frequencies
			freq_check = 1;
		} else if (summit_dfs_disable && drv->dfs_freq) {
			// only non-dfs frequency
			pfreq = drv->non_dfs_freq;
		}
		if (!pfreq) {
			// no frequency list needed
			break;
		}
		...
		if (freqs == NULL)
			goto fail;
		for (i = 0; *pfreq; pfreq++) {
			if (freq_check && !_sdc_allowed_freq(drv, *pfreq)) {
				wpa_printf(MSG_DEBUG, "nl80211: Scan excluding %u MHz", *pfreq);
				continue; // frequency excluded
			}
			if (nla_put_u32(msg, i++, *pfreq))
				goto fail;
			wpa_printf(MSG_MSGDUMP, "nl80211: Scan frequency %u "
				   "MHz", *pfreq);
		}
		if (i == 0) {
			wpa_printf(MSG_ERROR, "nl80211: scan failed as configured channels are disabled");
			goto fail;
		}
	...
	// thin-mac support for modifying scan timing
	if (summit_scan_dwell
		|| summit_scan_passive_dwell
		|| summit_scan_delay
		|| summit_scan_suspend_time)
	{
		if (0 == (drv->capa.flags2 & WPA_DRIVER_FLAGS2_SET_SCAN_DWELL)) {
			wpa_printf(MSG_DEBUG, "setting of scan dwell time"
					   "is not supported");
		} else {
			// note, the MEASUREMENT_DURATION is normally a single u16
			// kernel modification is required to support additional fields
			struct {
				// note, duration field is first for unmodified kernels
				u16 duration;
				u16 passive_channel_time;
				u16 probe_delay_time;
				u16 scan_suspend_time;
			} summit_scan_data;
			os_memset(&summit_scan_data, 0, sizeof(summit_scan_data));
			summit_scan_data.duration = summit_scan_dwell;
			summit_scan_data.passive_channel_time =
				summit_scan_passive_dwell;
			summit_scan_data.probe_delay_time =
				summit_scan_delay;
			summit_scan_data.scan_suspend_time =
				summit_scan_suspend_time;
			if (nla_put(msg, NL80211_ATTR_MEASUREMENT_DURATION,
						sizeof(summit_scan_data), &summit_scan_data))
				goto fail;
		}
	}
#endif
...
#ifdef _SDC_
// In some cases an AP interface may cause many inconsistent scan fetch results
// due to receiving all beacons.  An inconsistent scan fetch will occur if
// a split scan fetch is interrupted by a received beacon.
// Accept the results if the two most recent match for the list of bssids.
static int accept_inconsistent(struct wpa_scan_results *res0,
							   struct wpa_scan_results *res1)
{
	int i;
	if (res0->num != res1->num)
	{
		return 0;
	}
	for (i = 0; i < res0->num; i++) {
		if (!ether_addr_equal(res0->res[i]->bssid, res1->res[i]->bssid))
		{
			return 0;
		}
	}
...
nl80211_get_scan_results(struct wpa_driver_nl80211_data *drv, const u8 *bssid)
{
	...
	struct wpa_scan_results *res;
#ifdef _SDC_
	struct wpa_scan_results *res0 = NULL;
#endif
	int ret;
...
#ifdef _SDC_
		} else if (res0 && accept_inconsistent(res0, res)) {
			wpa_printf(MSG_DEBUG,
					   "nl80211: Accept inconsistent scan result dump");
			ret = 0;
#endif
		} else {
			wpa_printf(MSG_DEBUG,
