...
static int nl80211_mgmt_subscribe_non_ap(struct i802_bss *bss)
{
	...
	/* WNM - DMS Response */
	if (nl80211_register_action_frame(bss, (u8 *) "\x0a\x18", 2) < 0)
		ret = -1;
#endif
#endif /* CONFIG_WNM */
					...
					size_t csa_offs_len, int no_encrypt)
{
	...
	if (is_sta_interface(drv->nlmode) &&
		...
		if (freq == 0 &&
			...
			force_freq_zero = 1;
			wpa_printf(MSG_DEBUG,
					   "nl80211: send_mlme - Use freq=0 for external auth");
#else
...
#ifdef _SDC_
	if (force_freq_zero) {
		freq = 0; // should already be zero, see above
	}
	else {
#endif
	if (freq == 0) {
