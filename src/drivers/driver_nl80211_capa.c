					 ...
					 struct nlattr *tb)
{
...
#ifndef NL80211_EXT_FEATURE_PUNCT_DISABLED
	...
	if (ext_feature_isset(nla_data(tb), nla_len(tb),
						  NL80211_EXT_FEATURE_SET_SCAN_DWELL))
		capa->flags2 |= WPA_DRIVER_FLAGS2_SET_SCAN_DWELL;
#endif
}
...
static int wiphy_info_handler(struct nl_msg *msg, void *arg)
{
	...
	if (tb[NL80211_ATTR_MAX_NUM_PMKIDS]) {
		u32 max_num_pmkids = 0;
		max_num_pmkids =
			nla_get_u32(tb[NL80211_ATTR_MAX_NUM_PMKIDS]);
		capa->max_num_pmkids = max_num_pmkids;
	}
#endif
...
#ifdef _SDC_
static void
wpa_driver_nl80211_sdcprocess_modes(
	struct wpa_driver_nl80211_data *drv,
	struct hostapd_hw_modes *modes,
	u16 num_modes);
#endif
...
#ifdef _SDC_
		wpa_driver_nl80211_sdcprocess_modes(drv, result.modes, *num_modes);
#endif
...
#ifdef _SDC_
static int
_sdc_is_radar(struct wpa_driver_nl80211_data *drv,
			  struct hostapd_channel_data *channel)
{
	if (channel->flag & HOSTAPD_CHAN_RADAR)
		return 1;
	if (!drv->seen_radar_flag) {
		// check frequency for drivers that don't set the RADAR flag
		if (5260 <= channel->freq && channel->freq <= 5720)
			return 1;
	}
	return 0;
}
...
static void
_sdc_free_freq_lists(struct wpa_driver_nl80211_data *drv)
{
	os_free(drv->dfs_freq);
	drv->dfs_freq = NULL;
	os_free(drv->non_dfs_freq);
	drv->non_dfs_freq = NULL;
	os_free(drv->all_freq);
	drv->all_freq = NULL;
}

// create a list of dfs and non-dfs channels for scan channel selection
static void
wpa_driver_nl80211_sdcprocess_modes(
	struct wpa_driver_nl80211_data *drv,
	struct hostapd_hw_modes *modes,
	u16 num_modes)
{
	struct hostapd_hw_modes *band = NULL;
	int *pdfs;
	int *pnon;
	int *pall;
	int mode, i;
	int freq;
	int dfs_freq_count = 0;
	int non_dfs_freq_count = 0;
	int all_freq_count = 0;

	_sdc_free_freq_lists(drv);

	for (mode = 0; mode < num_modes; mode++) {
		if (!modes[mode].num_channels)
			continue;
		if (modes[mode].channels[0].freq < 50000) {
			band = &modes[mode];

			for (i=0; i<band->num_channels; i++) {
				if (band->channels[i].flag & HOSTAPD_CHAN_DISABLED)
					continue;
				all_freq_count++;
				if (_sdc_is_radar(drv, &band->channels[i])) {
					dfs_freq_count++;
					drv->seen_radar_flag = 1;
				} else {
					non_dfs_freq_count++;
				}
			}
	...
	drv->dfs_freq = os_calloc(dfs_freq_count + 1, sizeof(int));
	drv->non_dfs_freq = os_calloc(non_dfs_freq_count + 1, sizeof(int));
	drv->all_freq = os_calloc(all_freq_count + 1, sizeof(int));

	if (!drv->dfs_freq || !drv->non_dfs_freq || !drv->all_freq) {
		_sdc_free_freq_lists(drv);
		return;
	}

	pdfs = drv->dfs_freq;
	pnon = drv->non_dfs_freq;
	pall = drv->all_freq;
	...
	for (mode = 0; mode < num_modes; mode++) {
		...
		if (modes[mode].channels[0].freq < 50000) {
			...
			for (i=0; i<band->num_channels; i++) {
				if (band->channels[i].flag & HOSTAPD_CHAN_DISABLED)
					continue;
				freq = band->channels[i].freq;
				if (int_array_has(drv->all_freq, freq))
					continue;
				*pall++ = freq;
				if (dfs_freq_count && _sdc_is_radar(drv, &band->channels[i])) {
					*pdfs++ = freq;
					// disable DFS channels as long as there are still non-DFS channels present
					if (summit_dfs_disable && non_dfs_freq_count) {
						band->channels[i].flag |=
							(HOSTAPD_CHAN_DISABLED | HOSTAPD_CHAN_DISABLED_FOR_RADAR);
					}
				} else {
					*pnon++ = freq;
				}
